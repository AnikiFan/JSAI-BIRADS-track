...
if __name__ == '__main__':
    ...
    cla_files.sort(key=lambda x: int(os.path.basename(x).split('.')[0])) # 按照文件名中的数字排序
    cla_pre = pd.DataFrame({"img_name":cla_files})

    # 收集待测试图像路径，文件名按字典序排序
    fea_files = glob(os.path.join(fea_folder_path,'*')) # 收集fea数据集文件夹下的所有文件
    fea_files.sort(key=lambda x: int(os.path.basename(x).split('.')[0])) # 按照文件名中的数字排序
    fea_pre = pd.DataFrame({"img_name":fea_files})

    # 实例化用于测试的类
    tester = Tester(model_folder_path,format='pt')
